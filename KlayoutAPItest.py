...
def make_bone(cell,org,length,layer,vert=False,ps=.33,ol=None,two_sided=True,con_layer=None):
   if not con_layer:
      con_layer = layer + ".con"
   if vert:
      make_rect(cell,[org[0]-.085,org[1]-.085],[.17,.17],con_layer)
      if two_sided:
         make_rect(cell,[org[0]-.085,org[1]-.085+length],[.17,.17],con_layer)
      ...
      make_rect(cell,[org[0]-.085,org[1]-.085],[.17,.17],con_layer)
      if two_sided:
         make_rect(cell,[org[0]-.085+length,org[1]-.085],[.17,.17],con_layer)
...
sixteen_delay = layout.create_cell("sixteen_delay")
...
make_rect(sixteen_delay,[1.12,-1.38-.37-1-.11],[21.765,.17],"m2")

make_rect(sixteen_delay,[0.255,16.05],[14.95,.17],"m2")
make_rect(sixteen_delay,[0.255,16.05],[.17,-18.91],"m2")
...
make_rect(sixteen_delay,[7*.48*10-.54+.095+.1+.12+4.4,-1.5+16.7],[.32,.43],"m1")
make_rect(sixteen_delay,[7*.48*10-.54+.095+.1+.12+4.4,-2.4+16.7],[.32,.6],"m1")
...
make_rect(sixteen_delay,[37.83,6.5],[.25,.15],"poly")
make_rect(sixteen_delay,[38,6.0],[.33,.65],"poly")
make_rect(sixteen_delay,[38,6.0-.1],[.33,.85],"npc")
make_rect(sixteen_delay,[38+.08,6.08],[.17,.17],"li.con")
make_rect(sixteen_delay,[38-.2+.255,6.0],[.265,.33],"li")
make_bone(sixteen_delay,[38-.7,6.0],.88,"m1")

make_bone(sixteen_delay,[38.3,1.06],11.2,"m2",True,ps=.35,ol="m1")
make_bone(sixteen_delay,[38.3-.57,1.06+.85],11.2-.85*2,"m2",True,ps=.35,ol="m1")
make_bone(sixteen_delay,[36.11,-2.35],18.09,"m2",True,ps=.35,ol="m1")
make_bone(sixteen_delay,[36.61,-1.01],17.09-1.76,"m2",True,ps=.35,ol="m1")
...
make_rect(sixteen_delay,[9.55+19-.17,-1.41-.085],[.17,20-3.6],"m2")
make_rect(sixteen_delay,[9.55+21.5-.17,-1.41-.085-.6],[.17,20-3.6+.92],"m2")
make_rect(sixteen_delay,[9.55+21.5,-1.41-.085-.6+20-3.6+.92-.07],[-5,.17],"m2")

make_bone(eight_delay,[19.62,-1.44],8.2,"m2",True,ps=.35,ol="m1",two_sided=False)
make_rect(sixteen_delay,[19.695,-1.44+8.08],[-1.47,.17],"m2")
make_rect(sixteen_delay,[19.62,-2.0],[-.5,0.17],"m1")
make_bone(sixteen_delay,[19.12,-2+.085],8.2,"m2",True,ps=.35,ol="m1",two_sided=False)
make_bone(sixteen_delay,[16.64,15.21],-8.2-.725,"m2",True,ps=.35,ol="m1",two_sided=False)
make_rect(sixteen_delay,[19.12,-1.44+7.47],[-2.47-.105,.17],"m2")

make_rect(sixteen_delay,[36.6,15.98],[.17,.5],"m1")
make_rect(sixteen_delay,[37.5,15.98],[.17,.5],"m1")
...
dcell = db.DCellInstArray.new(sixteen_delay.cell_index(),db.DTrans.R0)
top.insert(dcell)

dcell = db.DCellInstArray.new(scs8hs_buf_2.cell_index(),db.DTrans.M90)
dcell.trans = db.DTrans.new(0,False,0.48*-2,14-.68) * dcell.trans
top.insert(dcell)
...
dcell = db.DCellInstArray.new(scs8hs_buf_2.cell_index(),db.DTrans.M90)
dcell.trans = db.DTrans.new(0,False,0.48*-2,0) * dcell.trans
top.insert(dcell)
...
dcell = db.DCellInstArray.new(scs8hs_buf_2.cell_index(),db.DTrans.M90)
dcell.trans = db.DTrans.new(0,False,0.48*-2,7-.34) * dcell.trans
top.insert(dcell)
...
make_rect(top,[-1.55,14.23],[1,.17],"m1")
make_rect(top,[-2.1,15.585],[1.3,.17],"m1")
make_bone(top,[-2.1,15.0],1-.16,"m1",vert=True,ol="li",two_sided=False)
make_bone(top,[-1.5,14.785],-1+.53,"m1",vert=True,ol="li",two_sided=False)
...
make_rect(top,[-1.3,5.1],[1.2,.17],"li")

make_bone(top,[-1.3+20.3,2.325],-20.4,"m1",ps=.35,ol="m2",con_layer="m2.con",two_sided=False)
